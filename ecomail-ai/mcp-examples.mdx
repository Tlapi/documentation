...
<AccordionGroup>
  ...
  <Accordion title="Campaigns sent in a period">
    ```
    List every campaign sent between 1 June and 31 August with its open rate, click rate and unsubscribe rate, sorted by open rate.
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Weekly trend">
    ```
    Show my bounce rate and spam complaint rate week by week for the last 90 days. Is it getting better or worse?
    ```
  </Accordion>
  <Accordion title="Breakdown by mailbox provider">
    ```
    Break down last month's deliverability by mailbox provider. Where is the bounce rate highest?
    ```
  </Accordion>
  <Accordion title="Breakdown by recipient domain">
    ```
    Which recipient domains had the most bounces in the last 30 days?
    ```
  </Accordion>
</AccordionGroup>
