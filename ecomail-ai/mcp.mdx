...
The local server runs through `npx`, so you need **Node.js 20 or later** installed. Verify your version with `node --version` — if the command isn't found or the version is lower, [download the latest LTS release](https://nodejs.org).

<Tip>
  Don't want to install anything? Use the [remote MCP server](/ecomail-ai/remote-mcp) instead — it runs in the cloud and needs no Node.js.
</Tip>
