...
<ParamField query="from_date" type="string">
  Filter events from this date onwards (`YYYY-MM-DD` in UTC). Cannot be more than one year in the past.
</ParamField>

<ParamField query="to_date" type="string">
  Filter events up to this date (`YYYY-MM-DD` in UTC). Must be after `from_date`.
</ParamField>

<ParamField query="detail" type="string">
  Set to `true` to return each individual event per subscriber (with `event`, `url` and `occured_at`) instead of aggregated per-event counts.
</ParamField>
