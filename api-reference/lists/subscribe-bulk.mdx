...
<ParamField body="subscriber_data" type="array" required>
  ...
  <Expandable title="subscriber object properties">
    ...
    <ParamField body="subscriber_data[][fcm_token]" type="string">
      Firebase Cloud Messaging (FCM) registration token used to deliver push notifications. Max 255 characters. Required whenever you send `fcm_app_id`.
    </ParamField>

    <ParamField body="subscriber_data[][fcm_platform]" type="string">
      Device platform for push notifications. Must be `android`, `ios`, or `web`. Can only be sent together with `fcm_token`.
    </ParamField>

    <ParamField body="subscriber_data[][fcm_app_id]" type="string">
      Firebase application ID associated with the FCM token. Max 150 characters. Required whenever you send `fcm_token`.
    </ParamField>
  </Expandable>
</ParamField>
    ...
    "subscriber_data": [
      {
        ...
        "fcm_token": "fcmExampleToken:APA91bExampleRegistrationToken",
        "fcm_platform": "ios",
        "fcm_app_id": "1:1234567890:ios:abc123def456"
      },
      {
        "name": "Foo 2",
        "email": "foo2@bar.cz"
